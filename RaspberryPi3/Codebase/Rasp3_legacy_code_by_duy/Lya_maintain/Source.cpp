...
int main(int argc, char *argv[])
{
	...
	while (true)
	{
		...
		else
		{
			...
			printf("Box 8, Box 9 : %f, %f", BoxDataXY[8], BoxDataXY[9]);
			Point vertices[] = { {BoxDataXY[4], BoxDataXY[5]}, {BoxDataXY[8], BoxDataXY[9]} ,{BoxDataXY[2], BoxDataXY[3]}, {BoxDataXY[6], BoxDataXY[7]} };  // Vi tri thung swap lai theo thu tu THUNG
			...
			if (dataTransfer_string.length() > 1)
				...
				force = std::stof(dataTransfer_string);
			else  force = 0;
			...
			else
			{
				...
				if (abs(theta) < (8 * 3.14 / 180))  omega = 0;  // Allowable angle error = 10 deg
					...
					omega = theta / elapsedTime + omega_offset; // van toc goc
				else if (theta < 0)
					omega = theta / elapsedTime - omega_offset;
				wL = (norm_v - omega * 0.1 * rotation_gain / 2.0) / 0.02;
				wR = (norm_v + omega * 0.1 * rotation_gain / 2.0) / 0.02;
