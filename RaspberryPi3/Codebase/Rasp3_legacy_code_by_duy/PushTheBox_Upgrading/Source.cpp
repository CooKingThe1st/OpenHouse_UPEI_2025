...
double norm_v =0;
float kp,ki,kd;
double PID_x_value,PID_y_value, previousPID_error,previousPID_error2,PID_errorTotal2,Rotation_robot, PID_errorTotal,wL,wR;
float rotation_gain=2;  // the higher parameter, the better rotation
...
int main(int argc, char *argv[])
{
	...
	if (string(argv[4]) == "-r") //  Normal mode
	{
	...
	while (true)
	{
		...
		else
		{
			...
			//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				// Rotation la goc heading cua Robot
				// angle  la goc di chuyen cua robot --- Robot's angle TOWARD desired point  movement
                                // theta -- Goc can xoay !! headding error
				...
				// From out PID -> using inverse kinematic of DD robot to calculate wL,wR
				//  we aim to both tranlate  and rotate , but emphasize more on rotation
				 norm_v= sqrt(PID_x_value*PID_x_value+PID_y_value*PID_y_value); 
                                if (norm_v>0) angle=atan2(PID_y_value,PID_x_value) ; else angle=rotation; theta= angle-rotation;
				
  	// Wrap theta to -pi -> pi
				 while (theta> 3.14) theta = theta -2*3.14;
				 while (theta<-3.14) theta = theta +2*3.14;

				if (abs(theta)>0.6*3.14) {     // Diem can di den  o phia sau,, di lui lai
  					theta_back = theta+ 3.14; 
                                        while (theta_back > 3.14) theta_back -= 2*3.14;
					while (theta_back <  -3.14) theta_back += 2*3.14;
			        if (abs(theta_back)>3.14/3)   theta_back=(theta_back/abs(theta_back))*3.14/3;   //do xoay bao hoa maximum
					   if (abs(theta_back) < (8*3.14/180))  omega = 0;  // Allowable angle error = 8 deg
						else omega=theta_back/elapsedTime; // van toc goc
						wL=-(norm_v+omega*0.05*rotation_gain)/0.02;
						wR=-(norm_v-omega*0.05*rotation_gain)/0.02;
				}
				else
				{
