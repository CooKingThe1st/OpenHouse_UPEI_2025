...
void denoiseImage(cv::Mat &inputImage, int cols, int rows)
{
	...
	erode(inputImage, inputImage, cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(2, 2)));
...
{
	circle(inputImage, cv::Point2f(float(imageWidth / 2), float(imageHeight / 2)), 140, cv::Scalar(0, 0, 0), -1);
}
