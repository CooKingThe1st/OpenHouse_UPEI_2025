...
double force_set = 0;
float vx; float vy;
int mode = 9; // mode =1: bam  theo hop  // mode = 0: behavior bassed, mode 3 is the final one in Viart paper // mode 4: force & contact control dựa vào góc va chạm Cascade control // mode5: Lyapunov thuần // mode 6: Lyapunove modified, robot DD thẳng đến goal //mode 7: robots bám đuôi// mode 8: thẳng, đụng và xoay
// mode = 9: Force based behavior - 4 robot// 2 push: push theo quy luật fcos - 2 observe: bám ngược!!! 
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	writeData << "Time" << "," << "Contact Angle" << "," << "Contact Force" << "," << "Number Contact"<< "," << "x_robot" << "," << "y_robot" << "," << "RB_frame" <<","<<" wL"<< "," << "wR" << "," << "force_set" << "," << "velo" << "\n";
	writeData.flush();
	...
	while (true)
	{
		...
		std:cout << "Desired force:-----------------------------------------------------" << force_set << endl;
			...
			if (numContact > 0)
			{
				...
				if ((robotID == 4) || (robotID == 3)) // robot 3 đẩy
				{
				//	force_set = 20 * error_vel + PID_errorTotal3 * 0.5;
					...
					force_set = 2.0;
					if (force_set > 4) force_set = 4;
					if (force_set < 0.4) force_set = 0.4;
					if (outForce[0] < 0) // skin lung qua sau.
						outForce[0] = 4.5;
					PID_error = force_set - outForce[0];
					PID_errorTotal = PID_errorTotal + PID_error;
			...
			else
			{
				...
				{
					force_set = 0.5; // bám cho lực nhỏ hơn
				}
