...
cv::Point2f illustrateVec_point2f(cv::Mat& inputImage, char left, char right, char middle, vector<cv::Point3f> savePoint, vector<cv::Point3f> changedPoint, float &angleOfContact)
{
	cv::Point2f intersectionPt_leftRight, middlePt_leftRight;
	cv::Point3f line_left, line_middle, line_right;
	cv::Point3f lineComb_leftRight;
	...
	//putText(inputImage, to_string(angleOfContact), Point2f(centerPt.x + 20 , centerPt.y + 20 ), cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 0, 255), 1, 8,false);
...
float estimateForce(float angle, float DisNearest)
{
	...
	float pow_angle5 = pow_angle2 * pow_angle3;
	float pow_angle4 = pow_angle2 * pow_angle2;
	float pow_dis4 = pow_dis2 * pow_dis2;
	float pow_dis5 = pow_dis2 * pow_dis3;

	force = p00 + p10 * angle + p01 * DisNearest + p20 * pow_angle2 +
		p11 * angle * DisNearest + p02 * pow_dis2 +
		p30 * pow_angle3 + p21 * pow_angle2 * DisNearest +
		p12 * angle * pow_dis2 + p03 * pow_dis3 +
		p40 * pow_angle4 + p31 * pow_angle3 * DisNearest +
		p22 * pow_angle2 * pow_dis2 + p13 * angle * pow_dis3 +
		p04 * pow_dis4 + p50 * pow_angle5 + p41 * pow_angle4 * DisNearest +
		p32 * pow_angle3 * pow_dis2 + p23 * pow_angle2 * pow_dis3 +
		p14 * angle * pow_dis4 + p05 * pow_dis5;
	...
	{ 
		if (displacement[contactList[i]].x != 0)
		{
			if (contactList[i] == (displacement.size() - 1))
			{
				...
				circlePoint_descar = illustrateVec_point2f(inputImage, left, right, middle, savePoint, changedPoint,_angle);
				...
				//_def = -0.1657 + 1.235*_deflection + (-0.1013)*_degree + -0.04737*_deflection*_deflection+0.04039*_degree*_deflection +0.01971*_degree*_degree+0.001084*_deflection*_deflection*_deflection +(-0.001265)*_deflection*_deflection*_degree+0.0001375*_deflection*_degree*_degree+(-0.000535)*_degree*_degree*_degree;
				//_F = 0.581*exp(0.0379*_def) + (-0.6531)*exp((-0.425)*_def);
				...
				//cout << " _def            : " << _def << endl;
				cout << " _Force          : " << _F << endl;
				//writeSth(writeFile,middle,right,left,savePoint,changedPoint,_angle,_def,_F);
			}
...
void writeSth(std::ofstream& writeFile, int middle, int right, int left, vector<cv::Point3f> changedPoint, vector<cv::Point3f> savePoint, float contactAngle, float deflection_mm, float contactForce)
{
	using namespace std::chrono;
	auto now = std::chrono::system_clock::now();
	auto ms = std::chrono::duration_cast<milliseconds>(now.time_since_epoch()) % 1000;
	...
	writeFile <<to_string(middle) << ","
		...
		<< " " + oss.str() << ","
		<< to_string(contactAngle) << ","
		<< to_string(deflection_mm)<< ","
		<< to_string(contactForce) << ",";
	*/	
	writeFile <<to_string(middle) << ","
		<< to_string(secondPoint) << ","
