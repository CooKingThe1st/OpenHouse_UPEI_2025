...
# Import waypoint bridge - need to copy this file to path planner directory
try:
    from waypoint_bridge import WaypointBridge
except ImportError:
    print("WARNING: waypoint_bridge.py not found - waypoint export will not work!")
    WaypointBridge = None
...
class MultiRobotGUI(QMainWindow):
    ...
    def create_drawing_controls(self) -> QGroupBox:
        ...
        # Generate straight path - Purple (creative/generate action)
        self.gen_straight_btn = QPushButton("Gen Straight Path")
        self.gen_straight_btn.setMinimumHeight(72)
        self.gen_straight_btn.setFont(QFont("Inter", 32, QFont.DemiBold))
        self.gen_straight_btn.setStyleSheet("""
            QPushButton {
                background-color: #8B5CF6; /* purple for generation */
                color: #FFFFFF;
                border: none;
                border-radius: 8px;
                padding: 12px 18px;
        ...
        """)
        self.gen_straight_btn.clicked.connect(self.generate_straight_paths)
        layout.addWidget(self.gen_straight_btn)
    ...
    def generate_straight_paths(self):
        """Generate dirt cheap straight-line paths from start to end for all robots."""
        if not self.current_mission:
            QMessageBox.warning(self, "No Mission", "Please load a mission first!")
            return
        
        # Clear existing paths
        self.canvas.clear_all_paths()
        
        self.log("=== Generating Straight Paths ===")
        
        # Generate straight paths for all robots
        for robot in self.current_mission.robots:
            color_key = robot.color.value
            
            # Create simple straight line from start to end
            start_x, start_y = robot.start_pos
            end_x, end_y = robot.end_pos
            
            # Generate 10 points along the straight line for smoothness
            straight_path = []
            for i in range(11):
                t = i / 10.0
            ...
            self.canvas.drawn_paths[color_key] = straight_path
            self.log(f"✓ {robot.display_name}: Straight path generated")
        
        # Draw the paths
        self.canvas.draw_mission()
        
        # Update status
        robot_count = len(self.current_mission.robots)
        self.status_label.setText(f"✓ Generated straight paths for {robot_count} robot(s). Click 'Set Waypoints' to continue.")
        self.canvas_info_bar.setText(f"✓ Straight paths generated for {robot_count} robot(s)")
        self.canvas_info_bar.setStyleSheet("""
            QLabel {
                background: #E0E7FF;
                color: #3730A3;
        ...
        QMessageBox.information(
            self,
            "Straight Paths Generated",
            f"✓ Generated straight-line paths for all {robot_count} robot(s)!\n\n"
            "• Fastest and simplest solution\n"
            "• No obstacle avoidance\n"
            "• Direct start-to-end route\n\n"
            "Click 'Set Waypoints' to optimize and prepare for sending."
        )
    
    def set_waypoints(self):
        """Generate optimized waypoints from drawn paths (includes validation)."""
        if not self.canvas.drawn_paths:
            QMessageBox.warning(self, "No Paths", "Please draw at least one path first!")
            return
        
        self.log("=== Validating and Setting Waypoints ===")
        ...
        self.log("✓ All paths validated successfully")
        
        # Step 2: Generate waypoints for each robot
        self.log("\n--- Step 2: Waypoint Generation ---")
        ...
        self.status_label.setText("✓ Paths validated & waypoints set! Review them, then click 'Send to Robots'.")
        QMessageBox.information(
            self,
            "Waypoints Generated",
            "✓ Paths validated and waypoints generated!\n\n"
    ...
    def send_to_robots(self):
        ...
        # === BRIDGE INTEGRATION: Save waypoints to JSON ===
        if WaypointBridge:
            # Map robot colors to IDs (blue=1, red=2, green=3, yellow=4)
            # color_to_id = {'blue': 1, 'red': 2, 'green': 3, 'yellow': 4}
            color_to_id = {'red':1, 'green':2, 'blue':3, 'yellow':4}  # Adjusted mapping    
            waypoints_by_id = {}
            
            for color_key, waypoints in robot_commands.items():
                robot_id = color_to_id.get(color_key, 1)
                # Convert to (order, x, y) format
                waypoints_by_id[robot_id] = [
                    (i, x, y) for i, (x, y) in enumerate(waypoints)
                ]
            
            if WaypointBridge.save(waypoints_by_id):
                self.log("\n✓ Waypoints saved to waypoints.json")
                self.log("✓ Ready to load in test_serial.py (press 'l')")
            else:
                self.log("\n✗ Failed to save waypoints to bridge file")
        else:
            self.log("\n⚠ WaypointBridge not available - waypoints not saved")
        ...
        QMessageBox.information(
            ...
            "• Saved to waypoints.json\n\n"
            "Switch to test_serial.py and press 'l' to load waypoints!"
        )
        
        self.status_label.setText("✓ Waypoints saved to waypoints.json - Load in test_serial.py (press 'l')")
