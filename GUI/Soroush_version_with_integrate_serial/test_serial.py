...
def cmd_prep():
    """Press 'j': PREP mode - send 20 waypoints (rectangle) once (1 per packet)."""
    global current_r_id
    ...
    # --- MODIFICATION: Updated print message
    print(f"\n[CMD_PREP] Robot {current_r_id + 1} - Sending {MAX_WAYPOINTS} rectangle waypoints (1 per packet, 1 pass)...")
    
    # Set PREP mode for the target slave
    slaves[current_r_id].current_cmd = CMD_PREP

    # --- MODIFICATION: Send 1 waypoint per packet ---
    
    packet_count = 0
    waypoints_per_packet = 1
    # num_packets is now just MAX_WAYPOINTS
    ...
    for packet_idx in range(num_packets):
        ...
        # (packet_idx is the same as the global waypoint index)
        global_wp_idx = packet_idx
        
        # Add the waypoint (5 bytes)
        order, x, y = waypoints[global_wp_idx]
        waypoint_data.extend(waypoint_to_bytes(order, x, y)) # 5 bytes
        
        # We now have 5 bytes of data.
        
        # Pad with 0s to fill the 14-byte data payload
        # (The packet function will only use the first 8)
        while len(waypoint_data) < 14:
            waypoint_data.append(0)
        
        # Set data (Data0-Data13)
        # self.data[0-4] will be the waypoint
        # self.data[5-7] will be 0 (padding)
        # These 8 bytes will be sent.
...
def print_menu():
    ...
    print(f"Logs: {SENT_LOG_FILE}, {RECEIVED_LOG_FILE}, {OPTITRACK_LOG_FILE}") # <-- MODIFIED
    print("\nKeyboard Commands:")
    print("  'a' : STOP current robot")
    print("  's' : Select next robot")
    print("  'f' : GO_TO origin (0, 0)")
    # --- MODIFICATION: Updated PREP description
    print("  'j' : PREP mode (sends 20 rectangle waypoints once, 1 per packet)") 
    print("  '9' : RUN operation")
    print("  'c' : CALIBRATE current robot (auto-finds offset)")  # <-- ADDED
